...
Generated cutouts automatically try progressively stronger edge cleanup, up to 110, and check the finished image before opening normal garment review. Uncertain edges are highlighted; light/dark backgrounds, zoom and optional **Adjust edges** remain available. Cleanup resumes from saved bytes without another generation charge, and background generation releases the storage writer so other items can be updated. Saved wardrobe images are not changed retroactively. See [automatic garment cleanup](docs/automatic-garment-cleanup.md) for checks, concurrency safeguards and exact-image validation.
