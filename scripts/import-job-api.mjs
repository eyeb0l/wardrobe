...
async function openAIEdit({ outsideLease = fn => fn(), telemetry, beforePaidCall, key, baseUrl, model, prompt, images, size, background, quality, timeoutMs }) {
  ...
  const { response, result } = await outsideLease(async () => {
...
async function openAIPlanModeledSetting({ outsideLease = fn => fn(), beforePaidCall, key, baseUrl, model, image, prompt, timeoutMs }) {
...
export function wardrobeImportApi(options = {}) {
    ...
    const task = (async () => {
      ...
      try {
        ...
        if (stageName === "garment") {
          ...
          // work from saved bytes without repeating the image request.
          return options.serverless ? publicJob(current) : await cleanGarment(current);
        } else {
          ...
          await saveJob(current);
          stage.setting = await openAIPlanModeledSetting({ outsideLease: options.outsideLease, beforePaidCall: options.beforePaidCall, timeoutMs, key, baseUrl: apiBaseUrl(), model: setting("OPENAI_VISION_MODEL", DEFAULT_VISION_MODEL), image: garment.data, prompt: stage.settingPrompt });
          ...
          await saveJob(current);
          bytes = await openAIEdit({ outsideLease: options.outsideLease, telemetry, beforePaidCall: beforeImageCall, timeoutMs, key, baseUrl: apiBaseUrl(), model: setting("OPENAI_MODELED_MODEL", setting("OPENAI_IMAGE_MODEL", DEFAULT_IMAGE_MODEL)), quality: setting("OPENAI_IMAGE_QUALITY", "high"), size: "1536x1024", images: [model, garment], prompt: stage.generationPrompt });
        }
  ...
  async function cleanGarment(job) {
  ...
  async function scheduleGeneration(job, stageName) {
    ...
    if (!["queued", "cleaning"].includes(stage.status) || typeof stage.taskId !== "string") {
    ...
    const task = {
      ...
      attempt: stage.attempts + (stage.status === "cleaning" ? 0 : 1), generationId: job.generationId || null,
    };
    try {
  ...
  async function runTask(task) {
    ...
    if (stage?.status === "cleaning" && task.stageName === "garment" && stage.taskId === task.taskId
      && task.attempt === stage.attempts && task.generationId === (job.generationId || null)) {
      return { skipped: false, job: await cleanGarment(job) };
    }
  ...
  async function failTask(task, message = "Generation was interrupted. Please retry this stage.") {
    ...
    const job = await loadJob(task.jobId);
    const stage = job?.stages?.[task.stageName];
    const expectedAttempt = stage?.attempts + (["processing", "cleaning"].includes(stage?.status) ? 0 : 1);
    if (!stage || !["pending", "queued", "processing", "cleaning"].includes(stage.status)
      || typeof task.taskId !== "string" || stage.taskId !== task.taskId
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      for (const stageName of ["garment", "modeled"]) {
        ...
        if (!stage || !["pending", "queued", "processing", "cleaning"].includes(stage.status)
          ...
          || (["processing", "cleaning"].includes(stage.status) && stage.attempts === 0)) continue;
        const attempt = stage.attempts + (["processing", "cleaning"].includes(stage.status) ? 0 : 1);
  ...
  async function handler(req, res, next) {
    ...
    try {
      ...
      if (action === "stages/garment/cleanup-auto" && req.method === "POST") {
        ...
        stage.cleanupSourceUrl ||= stage.failedAssetUrl;
        await saveJob(job);
        if (options.serverless) await scheduleGeneration(job, "garment");
        else void cleanGarment(job).catch(async error => { stage.status = "failed"; stage.error = error.message; await saveJob(job); });
      ...
      if (cleanupAction && req.method === "POST") {
        ...
        if (cleanupAction[1] === "cleanup-accept") {
          ...
          const cleaned = await (options.outsideLease || (fn => fn()))(async () => {
            const result = await processChromaBackground(source, key, { tolerance });
            return { ...result, verification: await inspectFinishedCutout(result.bytes, key) };
          });
      ...
      if (stageMatch && req.method === "POST") {
        ...
        if (stageName === "garment" && decision === "approve" && job.stages.modeled.status === "pending") {
          const input = await requestBody(req);
          if (input.previewUrl && input.previewUrl !== job.stages.garment.assetUrl) {
            return json(res, 409, { error: "The garment preview changed. Review the latest image before approving." });
          }
          const reference = await resolveModelReference(input.modelReferenceId ?? job.modelReferenceId ?? "default");
          job.modelReferenceId = reference.id;
        }
  ...
  return {
    ...
    async configResolved(config) {
      ...
      for (const id of ids) {
        ...
        if (job.stages.crop && job.stages.crop.status !== "approved") continue;
        if (job.stages.garment.status === "cleaning") {
          void cleanGarment(job).catch(async error => { job.stages.garment.status = "failed"; job.stages.garment.error = error.message; await saveJob(job); });
        } else if (["processing", "queued"].includes(job.stages.garment.status)) {
